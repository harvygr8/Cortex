...
export class GuardRailNode {
  constructor() {
    ...
    const prompt = ChatPromptTemplate.fromTemplate(`
      You are a content moderator focused only on detecting spam and foul language.
      Your task is to analyze the given question and determine if it contains inappropriate content.
      
      Guidelines for rejection:
      1. Spam content (repeated text, promotional content, etc.)
  ...
  async invoke(input, options = {}) {
    try {
      ...
      // If we have an onProgress callback, send the rejection message
