// Central color and font configuration - edit colors and fonts here
export const colors = {
  light: {
    background: 'bg-neutral-100',
    background2: 'bg-white',
    text: 'text-neutral-900',
    secondary: 'text-neutral-600',
    border: 'border-neutral-200',
    hover: 'hover:bg-neutral-100',
    accent: 'text-neutral-600',
    button: 'bg-neutral-800 text-white hover:bg-neutral-700',
    input: 'bg-white border-neutral-300 text-neutral-900',
    sidebar: 'bg-white border-neutral-200',
    sidebarIcon: 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900',
    sidebarLogo: 'bg-neutral-900 text-white',
    navbar: 'bg-white border-b border-neutral-200',
    warning: 'bg-amber-50 text-amber-800 border border-amber-200',
    danger: 'text-red-600',
    chatBubble: {
      user: 'bg-neutral-800 text-white shadow-mdr',
      ai: 'bg-white text-neutral-900 shadow-md'
    },
    statusIndicator: 'bg-green-500',
    statusText: 'text-green-600',
    focusRing: 'focus:ring-neutral-500',
    modal: {
      background: 'bg-white',
      text: 'text-neutral-900'
    },
    overlay: 'bg-black/50',
    dangerHover: 'hover:bg-red-50 hover:text-red-700',
    // Font classes - change these to update fonts globally
    font: {
      primary: 'font-ibm-plex-sans',
      heading: 'font-ibm-plex-sans',
      body: 'font-ibm-plex-sans',
      label: 'font-ibm-plex-sans'
    }
  },
  dark: {
    background: 'bg-neutral-900',
    background2: 'bg-neutral-800',
    text: 'text-neutral-100',
    secondary: 'text-neutral-400',
    border: 'border-neutral-700',
    hover: 'hover:bg-neutral-800',
    accent: 'text-neutral-400',
    button: 'bg-neutral-700 text-white hover:bg-neutral-600',
    input: 'bg-neutral-700 border-neutral-600 text-white',
    sidebar: 'bg-neutral-900 border-neutral-700',
    sidebarIcon: 'text-neutral-400 hover:bg-neutral-800 hover:text-white',
    sidebarLogo: 'bg-neutral-100 text-neutral-900',
    navbar: 'bg-neutral-900 border-b border-neutral-700',
    warning: 'bg-amber-900/30 text-amber-200 border border-amber-700',
    danger: 'text-red-400',
    chatBubble: {
      user: 'bg-neutral-700 text-white',
      ai: 'bg-neutral-800 text-neutral-100 border border-neutral-700'
    },
    statusIndicator: 'bg-emerald-500',
    statusText: 'text-emerald-400',
    focusRing: 'focus:ring-neutral-400',
    modal: {
      background: 'bg-neutral-800',
      text: 'text-neutral-100'
    },
    overlay: 'bg-black/50',
    dangerHover: 'hover:bg-red-900/30 hover:text-red-300',
    // Font classes - change these to update fonts globally
    font: {
      primary: 'font-ibm-plex-sans',
      heading: 'font-ibm-plex-sans',
      body: 'font-ibm-plex-sans',
      label: 'font-ibm-plex-sans'
    }
  }
};
